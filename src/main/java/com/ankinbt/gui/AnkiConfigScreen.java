...
public class AnkiConfigScreen extends Screen {
    ...
    private void rebuildButtons() {
        ...
        if (tab == Tab.GENERAL) {
            ...
            buttons.add(new UiBtn(left, y, rowW, rowH,
                    ...
                    () -> Minecraft.getInstance().setScreen(new CustomItemGroupsScreen(this)), true, null, true));
            y += rowH + gap;
        }
        ...
        if (tab == Tab.UI) {
            ...
            buttons.add(new UiBtn(left, y, rowW, rowH,
                    ...
                    () -> Minecraft.getInstance().setScreen(new ThemeCustomizeScreen(this)), true, null, true));
            y += rowH + gap;
    ...
    private void openControlsMenu() {
        ...
        try {
            mc.setScreen(new net.minecraft.client.gui.screens.options.controls.ControlsScreen(this, mc.options));
            return;
    ...
    public void render(LegacyGuiGraphics g, int mx, int my, float partialTick) {
        ...
        g.drawString(font, settingsIcon,
        ...
        g.drawString(font, tabTitle, px + 18, py + 64, accent, false);
        ...
        if (status != null && !status.getString().isEmpty() && System.currentTimeMillis() - statusTime < 2200) {
            g.drawString(font, status, px + 18, py + ph - 12, statusColor, false);
        }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        ...
        if (draggingPanel && button == 0) {
            px = Math.max(8, Math.min((int) Math.round(mouseX) - dragOffsetX, width - pw - 8));
            py = Math.max(8, Math.min((int) Math.round(mouseY) - dragOffsetY, height - ph - 8));
            contentTop = py + 74;
            contentBottom = py + ph - 44;
            rebuildButtons();
            return true;
    ...
    static class UiBtn {
        ...
        void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int accent, int offset, int contentTop, int contentBottom) {
            int yy = y + (scrollable ? offset : 0);
            ...
            g.drawString(font, text, x + 7, yy + (h - 8) / 2, color, false);
        }

        private static void drawEdge(LegacyGuiGraphics g, int x, int y, int w, int h, int color) {
