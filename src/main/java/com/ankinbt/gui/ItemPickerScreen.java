...
public class ItemPickerScreen extends Screen {
    ...
    private void rebuildButtons() {
        ...
        buttons.add(new UiBtn(xGroup, altY, wGroup, 20,
                ...
                () -> Minecraft.getInstance().setScreen(new CustomItemGroupsScreen(this)), true, null));
    ...
    private boolean handleKeyPressed(int key, int scanCode, int modifiers) {
        if (searchBox != null && searchBox.keyPressed(key, scanCode, modifiers)) {
            return true;
        }
        if (key == 256) {
            onClose();
    ...
    public void render(LegacyGuiGraphics g, int mx, int my, float partialTick) {
        ...
        g.drawString(font, Component.translatable("ankinbt.item_picker.tip"), px + 220, py + 13, TXT_DIM, false);

        if (searchBox != null) searchBox.render(g.unwrap(), mx, my, partialTick);

        for (UiBtn btn : buttons) btn.render(g, font, mx, my, accent);
        renderRows(g, mx, my);
    }

    private void renderRows(LegacyGuiGraphics g, int mx, int my) {
        int x = px + 18;
        int y = listY;
        int w = pw - 36;
        ...
        for (int i = start; i < end; i++) {
            ...
            g.drawString(font, nameDraw, textX, nameY, TXT_MAIN, false);
            int idX = textX + font.width(nameDraw) + 4;
            if (idX < textX + maxTextW) {
                g.drawString(font, idDraw, idX, nameY, TXT_DIM, false);
            }
        }
        g.disableScissor();
    ...
    private void renderScrollBar(LegacyGuiGraphics g, int x, int y, int h, int size, int rows, int scroll) {
        if (size <= rows) return;
    ...
    public boolean keyPressed(int key, int scanCode, int modifiers) {
        if (handleKeyPressed(key, scanCode, modifiers)) return true;
        return super.keyPressed(key, scanCode, modifiers);
    }

    @Override
    public boolean charTyped(char codePoint, int modifiers) {
        if (handleCharTyped(codePoint, modifiers)) return true;
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (handleMouseClick(mx, my, button)) return true;
