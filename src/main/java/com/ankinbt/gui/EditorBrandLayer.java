...
final class EditorBrandLayer {
    ...
    static void renderBackgroundLogo(LegacyGuiGraphics graphics, int width, int height) {
        graphics.blit(LOGO_TEXTURE, 9, Math.max(9, height - 41), 32, 32);
    }

    static void renderStatus(LegacyGuiGraphics graphics, Font font, int width, int height,
     ...
     */
    static void renderItemStatus(LegacyGuiGraphics graphics, Font font, int width, int height,
                                 float animation, String editorMode) {
    ...
    private static void renderStatusInternal(LegacyGuiGraphics graphics, Font font, int width, int height,
                                             float animation, String editorMode, float editorScale,
                                             float widthAdjustment, float heightAdjustment,
        ...
        String runtime = "Minecraft " + SharedConstants.getCurrentVersion().getName()
                + "  |  " + loader + " " + modVersion(loader.equals("Fabric") ? "fabricloader" : "neoforge", "unknown");
    ...
    static void renderSettingsButton(LegacyGuiGraphics graphics, Font font, int width, int mouseX, int mouseY,
    ...
    private static void drawTrimmed(LegacyGuiGraphics graphics, Font font, String value, int x, int y,
    ...
    private static void border(LegacyGuiGraphics graphics, int x, int y, int width, int height, int color) {
    ...
    private static String loaderName() {
        ...
        if (classPresent("net.neoforged.fml.ModList")) return "NeoForge";
        return "Unknown";
    }

    private static boolean classPresent(String name) {
        try {
            Class.forName(name, false, EditorBrandLayer.class.getClassLoader());
            return true;
    ...
    private static String modVersion(String id, String fallback) {
        try {
            ...
            Object optional = loaderType.getMethod("getModContainer", String.class).invoke(loader, id);
            if (optional instanceof java.util.Optional<?> value && value.isPresent()) {
                Object metadata = value.get().getClass().getMethod("getMetadata").invoke(value.get());
        ...
        try {
            ...
            Object optional = modListType.getMethod("getModContainerById", String.class).invoke(modList, id);
            if (optional instanceof java.util.Optional<?> value && value.isPresent()) {
                Object modInfo = value.get().getClass().getMethod("getModInfo").invoke(value.get());
                return String.valueOf(modInfo.getClass().getMethod("getVersion").invoke(modInfo));
            }
        } catch (Throwable ignored) {
        }
        return fallback;
