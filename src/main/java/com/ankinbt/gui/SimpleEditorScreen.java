...
public class SimpleEditorScreen extends Screen {
    ...
    private void renderMenuBar(LegacyGuiGraphics g, int mx, int my) {
        ...
        for (int i = 0; i < Category.values().length; i++) {
            ...
            if (tabW >= 46) {
                ...
                if (hover) g.renderTooltip(font, Component.literal(labels[i]), mx, my);
            }
        }
        ...
        if (advancedHover) g.renderTooltip(font,
    ...
    private void renderSizeControl(LegacyGuiGraphics g, int mx, int my) {
    ...
    private void renderSidebar(LegacyGuiGraphics g, int mx, int my) {
    ...
    private void renderCategoryContent(LegacyGuiGraphics g, int mx, int my) {
    ...
    private void renderCategoryCards(LegacyGuiGraphics g, int mx, int my, List<ActionRow> rows) {
        hoverRow = -1;
    ...
    private void renderCardActions(LegacyGuiGraphics g, ActionRow row, int mx, int my,
    ...
    private boolean renderLoreTip(LegacyGuiGraphics g, int mx, int my) {
    ...
    private boolean renderHomeTip(LegacyGuiGraphics g, int mx, int my) {
    ...
    public boolean mouseClicked(double mx, double my, int btn) {
        ...
        if (inventoryParent != null) {
            ...
            return inventoryParent.mouseClicked(mx, my, btn);
        }
    ...
    public boolean mouseReleased(double mx, double my, int button) {
        ...
        if (activeSubEditor != null
                && activeSubEditor.mouseReleased(mx, my, button,
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        if (activeSubEditor != null) return activeSubEditor.charTyped(codePoint, modifiers);
    ...
    private List<ActionRow> getGeneralRows() {
        ...
        if (isPotionLike()) {
            rows.add(new ActionRow(tr("ankinbt.simple.potion_base"), potionDisplayName(getPotionId()),
                    () -> activeSubEditor = new PotionPickerSubEditor(), accentColor(), null, null, ItemEditorVisuals.potionRowIcon(editStack.getItem().builtInRegistryHolder().key().location().toString())));
    ...
    private boolean supportsContainerPreview(ItemStack stack) {
        ...
        String itemId = "";
        try {
            itemId = stack.getItem().builtInRegistryHolder().key().location().toString();
        } catch (Throwable ignored) {}
    ...
    private void saveToItem() {
        ...
        if (InventoryEditorOverlay.isItemEditorPreviewMode()) {
            setStatus(tr("ankinbt.status.preview_only"), ERROR_C);
            return;
        }
        if (!mc.player.hasInfiniteMaterials()) {
            setStatus(tr("ankinbt.status.creative_only"), ERROR_C);
            return;
        }
        ...
        if (inventorySlot >= 0) {
            ...
            mc.gameMode.handleCreativeModeItemAdd(editStack.copy(), packetSlot(inventorySlot));
        } else {
    ...
    private void renderTextBuffer(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, TextEditBuffer buffer,
                                  int x, int y, int w, int h, boolean focused, String hint) {
        String value = buffer.value();
        if (value.isEmpty()) {
            g.drawString(font, hint == null ? "" : hint,
    ...
    class ConfirmResetSubEditor implements SubEditor {
        ...
        @Override
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my,
        ...
        private void renderResetButton(LegacyGuiGraphics g, net.minecraft.client.gui.Font font,
    ...
    class ConfirmCloseSubEditor implements SubEditor {
        ...
        @Override
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
    ...
    class NbtExportSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.drawString(font, tr("ankinbt.export.title"), dx + 12, dy + 9, UiTheme.textMain(), false);
            ...
            g.drawString(font, ".nbt", fieldX + fieldW - font.width(".nbt") - 4,
            ...
            g.drawString(font, tr("ankinbt.export.dir"), labelX + 7, pathY + 5, accentColor(), false);
            ...
            g.drawString(font, pathPreview, labelX + 7, pathY + 18, UiTheme.textDim(), false);

            int by = dy + dh - 27;
            if (message != null) {
                g.drawString(font, message, labelX, by - 13, msgColor, false);
            }
        ...
        private void renderExportLabel(LegacyGuiGraphics g, net.minecraft.client.gui.Font font,
                                       String label, int x, int y) {
            g.drawString(font, label, x, y + 6, UiTheme.textDim(), false);
        }

        private void renderExportInput(LegacyGuiGraphics g, net.minecraft.client.gui.Font font,
        ...
        private void renderExportButton(LegacyGuiGraphics g, net.minecraft.client.gui.Font font,
                                        int mx, int my, int x, int y, int w, int h,
    ...
    class ContainerPreviewSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            if (hoveredGlobal >= 0 && !hoveredStack.isEmpty()) {
                g.renderTooltip(font, hoveredStack.getHoverName(), mx, my);
            }
        }
        ...
        private void renderSmallBtn(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h, String text) {
    ...
    static class Btn {
        ...
        void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font f, int mx, int my) {
            ...
            if (hv && tooltip != null) g.renderTooltip(f, tooltip, mx, my);
        }
    }

    interface SubEditor {
        void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h);
    ...
    class InventorySwitchSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.drawString(font, tr("ankinbt.simple.inventory_pick"), dx + 10, dy + 8, UiTheme.textMain(), false);
            g.fill(dx + 1, dy + 22, dx + dw - 1, dy + 23, UiTheme.themedBorder(1f, 1f));
            g.drawString(font, tr("ankinbt.simple.inventory_hint"), dx + 10, dy + 30, UiTheme.textDim(), false);

            Minecraft mc = Minecraft.getInstance();
            if (mc.player == null) {
                g.drawString(font, tr("ankinbt.message.no_item"), dx + 10, dy + 48, ERROR_C, false);
                return;
            }
            ...
            for (int r = 0; r < ROWS; r++) {
                for (int c = 0; c < COLS; c++) {
                    ...
                    if (stack != null && !stack.isEmpty()) {
                        ...
                        if (hover) {
                            g.renderTooltip(font, stack.getHoverName(), mx, my);
                        }
                    }
                }
            }
            ...
            g.drawString(font, tr("ankinbt.edit.cancel"), bx + (bw - font.width(tr("ankinbt.edit.cancel"))) / 2, by + 5, UiTheme.textMain(), false);
        }
    ...
    class InlineFieldEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.drawString(font, title, dx + 10, dy + 8, UiTheme.textMain(), false);
            g.fill(dx + 1, dy + 22, dx + dw - 1, dy + 23, UiTheme.themedBorder(1f, 1f));
            ...
            g.drawString(font, disp, ix + 4, iy + 7, UiTheme.textMain(), false);
            ...
            if (colorEditable && !input.value().isEmpty()) {
                ...
                g.drawString(font, tr("ankinbt.simple.preview") + ": ", ix, iy + ih + 4, UiTheme.textDim(), false);
                int previewX = ix + font.width(tr("ankinbt.simple.preview") + ": ");
                g.drawString(font, preview, previewX, iy + ih + 4, UiTheme.textMain(), false);
            }

            if (error != null) g.drawString(font, error, ix, iy + ih + (colorEditable ? 16 : 4), ERROR_C, false);
            ...
            if (colorEditable) {
                ...
                g.drawString(font, palLabel, palX + (70 - font.width(palLabel)) / 2, palY + 4, UiTheme.textDim(), false);
            }
            ...
            g.drawString(font, tr("ankinbt.edit.cancel"), cancelX + (bw - font.width(tr("ankinbt.edit.cancel"))) / 2, by + 6, UiTheme.textDim(), false);

            int okX = dx + dw / 2 + 6;
            boolean oh = mx >= okX && mx < okX + bw && my >= by && my < by + bh;
            g.fill(okX, by, okX + bw, by + bh, oh ? accentColor() : 0xFF4F46E5);
            g.drawString(font, tr("ankinbt.edit.apply"), okX + (bw - font.width(tr("ankinbt.edit.apply"))) / 2, by + 6, UiTheme.textMain(), false);
        }
    ...
    implements SubEditor {
        ...
        @Override
        public void render(LegacyGuiGraphics g, Font font, int mx, int my, int x, int y, int w, int h) {
    ...
    class LoreColorInsertEditorForText implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.drawString(font, tr("ankinbt.simple.color_palette"), dx + 10, dy + 8, UiTheme.textMain(), false);
            g.fill(dx + 1, dy + 22, dx + dw - 1, dy + 23, UiTheme.themedBorder(1f, 1f));
            ...
            // Row labels
            g.drawString(font, tr("ankinbt.simple.palette_bright"), dx + 10, gridY - 1, UiTheme.textDim(), false);
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (hover) {
                    ...
                    g.drawString(font, tip, mx + 12, my - 12, UiTheme.textMain(), false);
                }
            }

            // Dark colors
            int darkY = gridY + cellH + 14;
            g.drawString(font, tr("ankinbt.simple.palette_dark"), dx + 10, darkY - 1, UiTheme.textDim(), false);
            ...
            g.drawString(font, tr("ankinbt.simple.format_codes"), dx + 10, fmtY, UiTheme.textDim(), false);
            fmtY += 14;
            int fmtX = gridX;
            String lang = Minecraft.getInstance().options.languageCode;
            ...
            for (int i = 0; i < MC_FORMAT_CODES.length; i++) {
                ...
                g.drawString(font, fLabel, fmtX + 7, fmtY + 5, hover ? UiTheme.textMain() : UiTheme.textDim(), false);
                fmtX += pillW + 6;
            }
            ...
            g.drawString(font, tr("ankinbt.simple.back"), backX + (backW - font.width(tr("ankinbt.simple.back"))) / 2, backY + 6, UiTheme.textDim(), false);
        }
    ...
    class PotionPickerSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.fill(dx, dy, dx + dw, dy + dh, UiTheme.surface(AnkiConfig.getUiOpacity(), openAnim));
            drawBorder(g, dx, dy, dw, dh, accentColor());
            g.drawString(font, tr("ankinbt.simple.pick_potion"), dx + 10, dy + 8, UiTheme.textMain(), false);
            g.fill(dx + 1, dy + 22, dx + dw - 1, dy + 23, UiTheme.themedBorder(1f, 1f));
            ...
            for (int i = scrollOff; i < end; i++) {
                ...
                g.drawString(font, label, dx + 36, ry + 5, selected ? UiTheme.textMain() : UiTheme.textDim(), false);
            }
    ...
    class PotionEffectSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.drawString(font, tr("ankinbt.simple.pick_effect"), l.dx() + 10, l.dy() + 8, UiTheme.textMain(), false);
            ...
            for (int i = scrollOff; i < end; i++) {
                ...
                g.drawString(font, label, l.leftX() + 32, ry + 6,
                        checked ? UiTheme.textMain() : UiTheme.textDim(), false);
                g.drawString(font, checked ? "-" : "+",
            ...
            g.drawString(font, tr("ankinbt.simple.potion_selected_list"),
            ...
            for (Map.Entry<String, EffectDraft> entry : selectedEffects.entrySet()) {
                ...
                g.drawString(font, label, l.rightX() + 32, ry + 6,
                        active ? UiTheme.textMain() : UiTheme.textDim(), false);
                g.drawString(font, draft.duration + " / " + draft.amplifier,
                        l.rightX() + l.rightW() - 28 - metaW, ry + 6, UiTheme.textDim(), false);
                g.drawString(font, "×", l.rightX() + l.rightW() - 17, ry + 6,
            ...
            if (activeEffectId == null || !selectedEffects.containsKey(activeEffectId)) {
                g.drawString(font, tr("ankinbt.simple.select_effect_first"),
                ...
                g.drawString(font, activeLabel,
                        l.rightX() + 6, formY, UiTheme.textMain(), false);
                g.drawString(font, tr("ankinbt.simple.effect_duration"),
                ...
                g.drawString(font, tr("ankinbt.simple.effect_amplifier"),
            ...
            g.drawString(font, confirm,
                    l.confirmX() + (l.buttonW() - font.width(confirm)) / 2, l.buttonY() + 6,
                    UiTheme.textMain(), false);
        }

        private void renderSmallTextBox(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int x, int y, int w, int h,
        ...
        private void renderToggle(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int bw, String label, boolean on) {
            ...
            g.drawString(font, text, x + (bw - font.width(text)) / 2, y + 5, on ? UiTheme.textMain() : UiTheme.textDim(), false);
        }
    ...
    class ColorPickerSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            g.drawString(font, String.format("#%06X", selectedColor & 0xFFFFFF), dx + 46, prevY + 6, UiTheme.textMain(), false);
            if (mode == -2) {
                String name = editStack.getHoverName().getString();
                ...
                g.drawString(font, previewName, dx + 46, prevY + 20, UiTheme.textMain(), false);
            }
    ...
    class AttributePickerSubEditor implements SubEditor {
        ...
        public void render(LegacyGuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            // Title
            g.drawString(font, tr("ankinbt.simple.pick_attr"), x + 8, y + 4, UiTheme.textMain(), false);
            ...
            for (int i = scrollOff; i < end; i++) {
                ...
                if (showNotes && getAttrNote(attrId) != null) {
                    ...
                    g.drawString(font, "i", infoX + 4, ry + 5, hovered ? UiTheme.textMain() : UiTheme.textDim(), false);
                }
            }
            ...
            if (showNotes) {
                ...
                if (note != null && !note.isBlank()) {
                    ...
                    g.drawString(font, "i", x + 14, noteY + 5, accentColor(), false);
                    g.drawString(font, shown, x + 26, noteY + 5, UiTheme.textDim(), false);
                } else {
                    g.drawString(font, tr("ankinbt.simple.attr_note_hint"), x + 10, noteY + 5, UiTheme.textDim(), false);
                }
            }
            ...
            // Amount input
            g.drawString(font, tr("ankinbt.simple.attr_amount"), x + 8, bottomY + 4, UiTheme.textDim(), false);
            ...
            // Operation selector
            int opY = bottomY + 22;
            g.drawString(font, tr("ankinbt.simple.attr_operation"), x + 8, opY + 4, UiTheme.textDim(), false);
            ...
            for (int i = 0; i < 3; i++) {
                ...
                g.drawString(font, opLabels[i], opX + 5, opY + 5, active ? UiTheme.textMain() : UiTheme.textDim(), false);
                opX += bw + 4;
            }

            // Slot selector
            int slotY = opY + 22;
            g.drawString(font, tr("ankinbt.simple.attr_slot"), x + 8, slotY + 4, UiTheme.textDim(), false);
            ...
            for (int i = 0; i < SLOT_KEYS.length; i++) {
                ...
                g.drawString(font, slotLabel, slotX + 4, slotY + 5, active ? UiTheme.textMain() : UiTheme.textDim(), false);
                slotX += bw + 3;
            ...
            g.drawString(font, tr("ankinbt.add.confirm"), confirmX + (70 - font.width(tr("ankinbt.add.confirm"))) / 2, confirmY + 6, UiTheme.textMain(), false);
        }
