...
final class ThemeCustomizeScreen extends Screen {
    ...
    public void render(net.minecraft.client.gui.GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        render(new LegacyGuiGraphics(graphics), mouseX, mouseY, partialTick);
    }

    private void render(LegacyGuiGraphics g, int mouseX, int mouseY, float partialTick) {
    ...
    private void renderField(LegacyGuiGraphics g, int x, int y, int w, int h, int index, String label,
    ...
    private void renderButton(LegacyGuiGraphics g, int x, int y, int w, String label, float hover, boolean primary, int accent) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        draggingField = false;
        if (inside(mouseX, mouseY, px + 16, py + 52, PANEL_W - 32, 26)) {
            activeField = 0;
            focusField(accentValue, mouseX - (px + 43), false);
            return true;
        }
        if (inside(mouseX, mouseY, px + 16, py + 92, PANEL_W - 32, 26)) {
            activeField = 1;
            focusField(backgroundValue, mouseX - (px + 43), false);
            return true;
        }
        int buttonY = py + PANEL_H - 34;
        int buttonW = 92;
        if (inside(mouseX, mouseY, px + PANEL_W / 2 - buttonW - 7, buttonY, buttonW, 22)) {
            UiSound.playClick();
            closeToParent();
            return true;
        }
        if (inside(mouseX, mouseY, px + PANEL_W / 2 + 7, buttonY, buttonW, 22)) {
            UiSound.playClick();
            applyColors();
            return true;
        }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (!draggingField || button != 0) return super.mouseDragged(mouseX, mouseY, button, dragX, dragY);
        TextEditBuffer buffer = activeField == 0 ? accentValue : backgroundValue;
        int px = (width - PANEL_W) / 2;
        int py = (height - PANEL_H) / 2 + Math.round((1f - openAnim) * 14f);
        int fieldY = activeField == 0 ? py + 52 : py + 92;
        if (mouseY >= fieldY - 6 && mouseY <= fieldY + 32) {
            buffer.moveTo(cursorFromMouse(buffer, (int) Math.round(mouseX) - (px + 43)), true);
        }
        return true;
    }
    ...
    public boolean keyPressed(int key, int scanCode, int modifiers) {
        ...
        TextEditBuffer buffer = activeField == 0 ? accentValue : backgroundValue;
        if (buffer.keyPressed(key, modifiers)) { error = ""; return true; }
        return super.keyPressed(key, scanCode, modifiers);
    }

    @Override
    public boolean charTyped(char codePoint, int modifiers) {
        char value = Character.toUpperCase(codePoint);
    ...
    private static void border(LegacyGuiGraphics g, int x, int y, int w, int h, int color) {
