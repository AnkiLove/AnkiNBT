...
[![Fabric](https://img.shields.io/badge/Fabric-supported-yellow?style=flat-square)](https://fabricmc.net)
...
```
AnkiNBT/
├── src/                    # Shared core code + NeoForge entry / 共享核心代码 + NeoForge 主入口
├── versions/               # NeoForge version adapters (1.21 ~ 1.21.11) / NeoForge 各版本适配
├── fabric/                 # Fabric main version (1.21.1) / Fabric 主版本
├── fabric-versions/        # Fabric version adapters (1.21 ~ 1.21.11) / Fabric 各版本适配
├── build.gradle
├── gradle.properties
└── README.md
```
...
Other versions: enter the corresponding `versions/<version>` or `fabric-versions/<version>` directory and run `gradlew build`.

其他版本同理，进入对应目录执行 `gradlew build` 即可。编译产物在各子目录的 `build/libs/` 下。

Note: Proxy settings in `gradle.properties` are for local dev environment. Modify or remove before building.
