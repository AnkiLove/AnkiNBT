...
public class SimpleEditorScreen extends Screen {
    ...
    class InlineFieldEditor implements SubEditor {
        ...
        InlineFieldEditor(String field, String currentValue, boolean isLore) {
            ...
            this.inputBox = new EditBox(SimpleEditorScreen.this.font, 0, 0, 1, 22, Component.empty());
        ...
        @Override
        public boolean charTyped(char c, int mod) {
            if (inputBox.charTyped(new CharacterEvent(c, mod))) return true;
            return true;
        }
    ...
    implements SubEditor {
        ...
        private final List<EditBox> lineBoxes = new ArrayList<EditBox>();
        ...
        public void render(GuiGraphics g, Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            for (int i = this.scrollOff; i < end; ++i) {
                ...
                EditBox box = this.lineBoxes.get(i);
        ...
        private EditBox newLineBox(String value) {
            EditBox box = new EditBox(SimpleEditorScreen.this.font, 0, 0, 1, 14, Component.empty());
        ...
        private void configureLineBox(EditBox box, int x, int y, int w, int h, boolean focused) {
            box.setX(x);
            box.setY(y);
        ...
        private boolean hasLineSelection(EditBox box) {
        ...
        public boolean mouseClicked(double mx, double my, int btn, int x, int y, int w, int h) {
            ...
            if (mx >= (double)textX && mx < (double)(textX + textW) && my >= (double)textY && my < (double)(textY + textH)) {
                ...
                if (clickedLine >= 0 && clickedLine < this.lines.size()) {
                    ...
                    EditBox box = this.lineBoxes.get(clickedLine);
        ...
        public boolean charTyped(char c, int mod) {
            EditBox box = this.activeBox();
            if (box.charTyped(new CharacterEvent(c, mod))) {
                this.syncLine(this.activeLine);
                return true;
            }
            this.syncLine(this.activeLine);
    ...
    class PotionPickerSubEditor implements SubEditor {
        ...
        PotionPickerSubEditor() {
            searchBox = new EditBox(SimpleEditorScreen.this.font, 0, 0, 1, 18, Component.empty());
    ...
    class PotionEffectSubEditor implements SubEditor {
        ...
        private final EditBox amplifierBox;
        private final List<String> filtered = new ArrayList<>();
        ...
        private EditBox numericBox(String value) {
            EditBox box = new EditBox(SimpleEditorScreen.this.font, 0, 0, 1, 18, Component.empty());
            box.setMaxLength(8);
        ...
        public void render(GuiGraphics g, net.minecraft.client.gui.Font font, int mx, int my, int x, int y, int w, int h) {
            ...
            renderSmallEditBox(g, searchBox, sx, sy, sw, sh, focusField == 0, mx, my);
            ...
            if (activeEffectId == null || !selectedEffects.containsKey(activeEffectId)) {
                ...
                renderSmallEditBox(g, durationBox, durX, editY, fieldW, 18, focusField == 1, mx, my);
                ...
                renderSmallEditBox(g, amplifierBox, ampX, editY, fieldW, 18, focusField == 2, mx, my);
        ...
        private void renderSmallEditBox(GuiGraphics g, EditBox box, int x, int y, int w, int h, boolean focused, int mx, int my) {
            box.setX(x);
            box.setY(y);
            box.setWidth(w);
            box.setFocused(focused);
        ...
        public boolean keyPressed(int key, int scan, int mod) {
            ...
            if (focusField == 0) {
                ...
                EditBox box = focusField == 1 ? durationBox : amplifierBox;
        ...
        public boolean charTyped(char c, int mod) {
            if (focusField == 0) {
                ...
                (focusField == 1 ? durationBox : amplifierBox).charTyped(new CharacterEvent(c, mod));
                syncActiveDraft();
                return true;
            }
            return true;
        ...
        public boolean charTyped(char c, int mod) {
            if (c >= 32) {
                if (focusLevel) {
                    ...
                    searchBox.charTyped(new CharacterEvent(c, mod)); searchQ = searchBox.getValue();
                }
                return true;
            }
            return false;
