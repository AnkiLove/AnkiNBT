...
public class SimpleEditorScreen extends Screen {
    ...
    private boolean isLeatherArmor() { return editStack.getItem().builtInRegistryHolder().key().location().toString().contains("leather_"); }
    ...
    private void copyGiveCommand() { var opt = NbtHelper.serializeItemStack(editStack); if (opt.isPresent()) { String id = editStack.getItem().builtInRegistryHolder().key().location().toString(); Minecraft.getInstance().keyboardHandler.setClipboard("/give @s " + id + " " + editStack.getCount()); setStatus(tr("ankinbt.simple.cmd_copied"), SUCCESS); } }
    ...
    private void applyEnchantLevel(String enchId, int level) {
        ResourceLocation loc = ResourceLocation.tryParse(enchId); if (loc == null) return;
