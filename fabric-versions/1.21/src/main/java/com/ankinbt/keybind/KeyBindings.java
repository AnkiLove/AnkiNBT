...
public class KeyBindings {
    ...
    private static KeyMapping openVillagerEditorKey;
    ...
    public static void register() {
        ...
        int villagerKeyCode = normalizeVillagerEditorKeyCode(AnkiConfig.getOpenVillagerEditorKeyCode(), entityKeyCode);
        if (entityKeyCode != AnkiConfig.getOpenEntityEditorKeyCode()) {
            AnkiConfig.setOpenEntityEditorKeyCode(entityKeyCode);
        }
        if (villagerKeyCode != AnkiConfig.getOpenVillagerEditorKeyCode()) {
            AnkiConfig.setOpenVillagerEditorKeyCode(villagerKeyCode);
        }
        ...
        openVillagerEditorKey = KeyBindingHelper.registerKeyBinding(new KeyMapping(
                "key.ankinbt.open_villager_editor",
                InputConstants.Type.KEYSYM,
                villagerKeyCode,
                CATEGORY
        ));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (shouldSkipSeparateVillagerBinding()) {
                while (openVillagerEditorKey.consumeClick()) {
                    // Drain duplicated presses when villager editing is merged into the smart entity key.
                }
                return;
            }

            while (!shouldSkipSeparateVillagerBinding() && openVillagerEditorKey.consumeClick()) {
                Entity looked = getLookedEntity(client);
                if (isVillagerEntity(looked)) {
                    client.setScreen(VillagerTradeEditorScreen.forEntity(looked, client.screen));
                    continue;
                }

                ItemStack held = getHeldOrOffhand(client);
                if (SpawnEggEditorHelper.isVillagerSpawnEgg(held)) {
                    client.setScreen(VillagerTradeEditorScreen.forSpawnEgg(held, -1, client.screen));
                    continue;
                }

                client.player.displayClientMessage(
                        net.minecraft.network.chat.Component.translatable("ankinbt.villager.target_hint"), true);
            }
        });
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            if (screen instanceof AbstractContainerScreen<?> containerScreen) {
                ScreenKeyboardEvents.beforeKeyPress(screen).register((scr, key, scancode, modifiers) -> {
                    ...
                    if (!shouldSkipSeparateVillagerBinding()
                            && openVillagerEditorKey.matches(key, scancode)
                            && SpawnEggEditorHelper.isVillagerSpawnEgg(stack)) {
                        client.setScreen(VillagerTradeEditorScreen.forSpawnEgg(stack, slotIndex, screen));
                    }
                });
            }
        });
    ...
    private static int normalizeEntityEditorKeyCode(int keyCode) {
        return keyCode == InputConstants.KEY_M ? InputConstants.KEY_COMMA : keyCode;
    }

    private static int normalizeVillagerEditorKeyCode(int keyCode, int entityKeyCode) {
        if (keyCode == InputConstants.KEY_V || keyCode == InputConstants.KEY_M) {
            return entityKeyCode;
        }
        return keyCode;
    }

    private static boolean shouldSkipSeparateVillagerBinding() {
        return AnkiConfig.isSmartEntityEditorKey()
                && keyCode(openEntityEditorKey, AnkiConfig.getOpenEntityEditorKeyCode())
                == keyCode(openVillagerEditorKey, AnkiConfig.getOpenVillagerEditorKeyCode());
    }
    ...
    public static boolean syncConfigFromKeyMappings() {
        ...
        int villagerCode = keyCode(openVillagerEditorKey, AnkiConfig.getOpenVillagerEditorKeyCode());
        if (villagerCode != AnkiConfig.getOpenVillagerEditorKeyCode()) {
            AnkiConfig.setOpenVillagerEditorKeyCode(villagerCode);
            changed = true;
        }
