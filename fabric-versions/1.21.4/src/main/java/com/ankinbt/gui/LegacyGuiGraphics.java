...
/** Normalizes the 1.21.4 renderer to the editor's small drawing surface. */
final class LegacyGuiGraphics {
    private final GuiGraphics delegate;
    LegacyGuiGraphics(GuiGraphics delegate) { this.delegate = delegate; }
    GuiGraphics unwrap() { return delegate; }
    ...
    void renderItem(ItemStack stack, int x, int y) { delegate.renderItem(stack, x, y); }
    void renderItem(ItemStack stack, int x, int y, int seed) { delegate.renderItem(stack, x, y, seed); }
    void renderItemDecorations(Font font, ItemStack stack, int x, int y) { delegate.renderItemDecorations(font, stack, x, y); }
    void blit(ResourceLocation texture, int x, int y, int width, int height) {
        delegate.blit(RenderType::guiTextured, texture, x, y, 0.0f, 0.0f, width, height, width, height);
    }
    void renderTooltip(Font font, Component tooltip, int x, int y) { delegate.renderTooltip(font, tooltip, x, y); }
    void renderTooltip(Font font, ItemStack stack, int x, int y) { delegate.renderTooltip(font, stack, x, y); }
}
