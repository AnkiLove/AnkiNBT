...
public class NbtEditorScreen extends Screen {
    ...
    public NbtEditorScreen(ItemStack stack) {
        ...
        this.fullItemTag = opt.orElseGet(() -> {
            ...
            fallback.putString("id", stack.getItem().builtInRegistryHolder().key().identifier().toString());
    ...
    private void buildButtons() {
        ...
        bx -= bw; buttons.add(new Btn(bx, by, bw, bw, "X", Component.translatable("ankinbt.btn.close"), this::tryClose));
    ...
    private void exportNbt() {
        String itemId = originalStack.getItem().builtInRegistryHolder().key().identifier().getPath();
