...
public class KeyBindings {
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            if (screen instanceof AbstractContainerScreen<?> containerScreen) {
                ScreenKeyboardEvents.beforeKeyPress(screen).register((scr, keyEvent) -> {
                    if (openEditorKey.matches(keyEvent)) {
    ...
    private static Slot getHoveredSlot(AbstractContainerScreen<?> screen) {
        if (hoveredSlotField != null) {
            try { return (Slot) hoveredSlotField.get(screen); } catch (Exception ignored) {}
        }
        return null;
    }
    ...
    private static void openEditor(ItemStack stack, int slot) {
        ...
        if (useSimpleMode) mc.setScreen(new SimpleEditorScreen(stack, slot));
        else mc.setScreen(new NbtEditorScreen(stack));
    }
}
